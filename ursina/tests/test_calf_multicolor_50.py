"""
Test CALF Policy - 50 Points with MultiColor Trails
===================================================

Phase 8.4: Визуализация 50 точек с мультицветными траекториями.

Демонстрирует:
1. 50 агентов с CALF политикой
2. Оптимизированная визуализация множества траекторий
3. Производительность с большим количеством агентов
"""
...
print("\n" + "="*70)
print("TEST: CALF Policy - 50 Points with MultiColor Trails")
print("="*70)
print(f"\nConfiguration:")
...
# Уменьшенный scale для 50 агентов
scale = max(0.06, 0.12 - (N_AGENTS / 1000))
...
for i in range(N_AGENTS):
    ...
    # MultiColor Trail (оптимизированные параметры для 50 агентов)
...
instructions_text = Text(
    text='[Phase 8.4] 50 agents - optimized multicolor trails',
    position=(0, -0.45),
    scale=0.9,
    color=Vec4(0.7, 0.7, 0.7, 1),
    origin=(0, 0)
...
def update():
    ...
    # Update statistics
    stats_text.text = f'''Phase 8.4: MultiColor Trails (50 Points)
...
print("   ESC or Q: Quit")
print("\nWatch the trails:")
print("   - 50 agents with multicolor trails")
print("   - ORANGE: Far from goal (Fallback mode)")
print("   - GREEN: Medium distance (Relax mode)")
print("   - BLUE: Close to goal (TD3 mode)")
print("\nPerformance optimizations:")
print("   - decimation=2 (every 2nd point)")
print("   - rebuild_frequency=10")
print("   - max_length=800")
print()
